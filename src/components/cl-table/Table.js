...
import VerticalDropdown from "../cl-dropdown/VerticalDropDownMenu";
...
class Table extends Component {
  ...
  componentDidUpdate(prevProps) {
    let sameProps = _.isEqual(
      prevProps.defaultPerPage,
      this.props.defaultPerPage
    );
    sameProps =
      sameProps && _.isEqual(prevProps.defaultPage, this.props.defaultPage);
    sameProps = sameProps && _.isEqual(prevProps.items, this.props.items);
    sameProps =
      sameProps && _.isEqual(prevProps.searchWord, this.props.searchWord);
  ...
  sortItemsByColumn = (column, sortOrder) => {
    ...
    if (this.state.pagination) {
      filteredItems = sortedItems.splice(
        0,
        this.state.selectedPaginationOption
      );
    }
    this.setState({
  ...
  navigateToAPage = (pageNumber, perPage) => {
    const rowsPerPage = perPage || this.state.selectedPaginationOption;
    let filteredItems = _.cloneDeep(this.state.items);
    let itemStart = (pageNumber - 1) * rowsPerPage;
    let itemEnd = itemStart + rowsPerPage;
    filteredItems = filteredItems.slice(itemStart, itemEnd);
    this.setState(
      {
  ...
  navigateToPreviousPage = () => {
    let filteredItems = _.cloneDeep(this.state.items);
    let itemStart =
      (this.state.pageIndex - 1) * this.state.selectedPaginationOption;
    let itemEnd = itemStart + this.state.selectedPaginationOption;
    filteredItems = filteredItems.slice(itemStart, itemEnd);
    this.setState(
      {
        pageIndex: this.state.pageIndex - 1,
      ...
      updateBroswerURL(this.props.history, {
        search: {
          ...
          per_page: this.state.selectedPaginationOption
        }
      })
    );
  };
  navigateToNextPage = () => {
    ...
    let fi = filteredItems.slice(itemStart, itemEnd);
    this.setState(
      { pageIndex: this.state.pageIndex + 1, filteredItems: fi },
      updateBroswerURL(this.props.history, {
        search: {
          page: this.state.pageIndex,
          per_page: this.state.selectedPaginationOption
  ...
  selectAll = event => {
    ...
    let itemStart = this.state.pageIndex * this.state.selectedPaginationOption;
  ...
  render() {
    ...
    return (
      <div key={'id="CLUITable"'}>
        ...
        >
          ...
          <div className="tableflex--innerflex">
            ...
            {this.props.selectable ? (
              <div className="tableflex__dropdown">
                <VerticalDropdown
                  data={this.selectedRows()}
                  options={this.props.tableActions}
                />
              </div>
            ) : null}
          </div>
        ...
        {this.state.pagination && this.state.searchWord === "" ? (
          <TablePaginator
            ...
            selectedPaginationOption={this.state.selectedPaginationOption}
